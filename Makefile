# This Makefile is for the sendxmpp extension to perl.
#
...
#     ABSTRACT => q[Script for sending xmpp messages]
#     AUTHOR => q[Dirk-Jan C. Binnema (djcb@djcbsoftware.nl)]
#     EXE_FILES => [q[sendxmpp]]
#     NAME => q[sendxmpp]
#     PREREQ_PM => { Getopt::Long=>q[2], Net::XMPP=>q[1] }
#     VERSION_FROM => q[sendxmpp]
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=1 --perm_rw=$(PERM_RW) \
	  sendxmpp $(INST_MAN1DIR)/sendxmpp.$(MAN1EXT) 
...
pure_all :: $(INST_SCRIPT)/sendxmpp
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/sendxmpp 

$(INST_SCRIPT)/sendxmpp : sendxmpp $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/sendxmpp
	$(CP) sendxmpp $(INST_SCRIPT)/sendxmpp
	$(FIXIN) $(INST_SCRIPT)/sendxmpp
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/sendxmpp
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               sendxmpp' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.0.8' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           Script for sending xmpp messages' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Dirk-Jan C. Binnema (djcb@djcbsoftware.nl)' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    Getopt::Long:  2' >> META_new.yml
	$(NOECHO) $(ECHO) '    Net::XMPP:     1' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0.0.8">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Script for sending xmpp messages</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Dirk-Jan C. Binnema (djcb@djcbsoftware.nl)</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Getopt::Long" VERSION="2" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Net::XMPP" VERSION="1" />' >> $(DISTNAME).ppd
